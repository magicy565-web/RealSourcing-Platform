-- ============================================================================
-- RealSourcing Factory Module Schema Upgrade
-- Version: 3.1
-- Date: 2026-02-21
-- Purpose: Add 24 new fields to support GTM 3.1 strategy
-- ============================================================================

-- ─── Step 1: Backup existing factories table ───────────────────────────────
-- Run this FIRST to ensure data safety
CREATE TABLE IF NOT EXISTS factories_backup_20260221 AS 
SELECT * FROM factories;

-- ─── Step 2: Add new fields to factories table (P0 & P1) ───────────────────

-- P0: Real-time status fields
ALTER TABLE factories ADD COLUMN isOnline TINYINT NOT NULL DEFAULT 0 COMMENT '当前是否在线';
ALTER TABLE factories ADD COLUMN lastOnlineAt DATETIME(3) COMMENT '最后在线时间';
ALTER TABLE factories ADD COLUMN availableForCall TINYINT NOT NULL DEFAULT 0 COMMENT '是否可连线';
ALTER TABLE factories ADD COLUMN averageResponseTime INT DEFAULT 0 COMMENT '平均响应时间（分钟）';

-- P0: Video & Certification fields
ALTER TABLE factories ADD COLUMN hasReel TINYINT NOT NULL DEFAULT 0 COMMENT '是否有Reel视频';
ALTER TABLE factories ADD COLUMN videoVerificationUrl VARCHAR(500) COMMENT '视频验厂URL';
ALTER TABLE factories ADD COLUMN certificationStatus VARCHAR(20) DEFAULT 'pending' COMMENT '认证状态（verified/pending/rejected）';
ALTER TABLE factories ADD COLUMN certificationDate DATETIME(3) COMMENT '最后认证日期';

-- P1: Operational data fields
ALTER TABLE factories ADD COLUMN viewCount INT NOT NULL DEFAULT 0 COMMENT '工厂页面浏览量';
ALTER TABLE factories ADD COLUMN favoriteCount INT NOT NULL DEFAULT 0 COMMENT '被收藏次数';
ALTER TABLE factories ADD COLUMN responseRate DECIMAL(5,2) DEFAULT 0.00 COMMENT '消息回复率（百分比）';
ALTER TABLE factories ADD COLUMN languagesSpoken JSON COMMENT '支持的语言列表';
ALTER TABLE factories ADD COLUMN isFeatured TINYINT NOT NULL DEFAULT 0 COMMENT '是否精选推荐';
ALTER TABLE factories ADD COLUMN featuredUntil DATETIME(3) COMMENT '精选推荐截止时间';
...
ALTER TABLE factories ADD INDEX idx_isOnline (isOnline);
ALTER TABLE factories ADD INDEX idx_certificationStatus (certificationStatus);
ALTER TABLE factories ADD INDEX idx_isFeatured (isFeatured);
ALTER TABLE factories ADD INDEX idx_viewCount (viewCount);
ALTER TABLE factories ADD INDEX idx_favoriteCount (favoriteCount);
...
--   - P1 (Operations): viewCount, favoriteCount, responseRate, languagesSpoken, isFeatured, featuredUntil
--
